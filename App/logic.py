...
def add_book_tag(catalog, book_tag):
    ...
    book_tag_value = lp.get(
        catalog["book_tags"],
        t["tag_id"]
    )

    if book_tag_value is not None:
...
def get_books_by_tag(catalog, tag_name):
    ...
    for book_tag in book_tags["elements"]:

        goodreads_book_id = book_tag["book_id"]
